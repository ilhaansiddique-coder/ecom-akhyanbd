...
import { FiSave, FiPlus, FiTrash2, FiChevronDown, FiChevronUp, FiUpload, FiX } from "react-icons/fi";
...
export default function HomepagePage() {
  ...
  useEffect(() => {
    api.admin.getSettings().then((res: any) => {
      const settings = Array.isArray(res) ? res : res.data || [];
      const hc = settings.find((s: any) => s.key === "homepage_content");
      if (hc?.value) {
        try {
          const parsed = JSON.parse(hc.value);
          setContent({ ...DEFAULT_CONTENT, ...parsed, hero: { ...DEFAULT_CONTENT.hero, ...parsed.hero }, reviews: { ...DEFAULT_CONTENT.reviews, ...parsed.reviews } });
        } catch { /* */ }
      }
      setLoading(false);
            ...
            <div className="p-5 space-y-4 border-t border-gray-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={content.hero.badge} onChange={e => updateHero("badge", e.target.value)} className={theme.input} placeholder="১০০% প্রাকৃতিক ও ভেষজ" />
                </div>
                <div>
                  <label className={theme.label}>Title</label>
                  <input value={content.hero.title} onChange={e => updateHero("title", e.target.value)} className={theme.input} placeholder="মা ভেষজ বাণিজ্যালয়" />
                </div>
                <div>
                  <label className={theme.label}>Subtitle</label>
                  <input value={content.hero.subtitle} onChange={e => updateHero("subtitle", e.target.value)} className={theme.input} placeholder="প্রাকৃতিক ভেষজ পণ্য" />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea value={content.hero.description} onChange={e => updateHero("description", e.target.value)} className={theme.textarea} rows={3} placeholder="প্রকৃতির শক্তিতে সুস্থ থাকুন..." />
              </div>
              <div>
                <label className={theme.label}>Secondary Button</label>
                <input value={content.hero.cta_secondary} onChange={e => updateHero("cta_secondary", e.target.value)} className={theme.input} placeholder="সব পণ্য দেখুন" />
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className={theme.label}>Trust Badge 1</label>
                  <input value={content.hero.trust_1} onChange={e => updateHero("trust_1", e.target.value)} className={theme.input} placeholder="১০০% ভেষজ" />
                </div>
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      {t.avatar ? (
                        ...
                        <label className="flex items-center gap-2 px-3 py-2 border border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-[#0f5931] transition-colors">
