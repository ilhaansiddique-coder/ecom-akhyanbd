...
export default function OrdersPage() {
  ...
  // Shipping zones
  const [shippingZones, setShippingZones] = useState<{ id: number; name: string; rate: number }[]>([]);
  useEffect(() => {
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2 md:gap-3 items-center">
            ...
            <button type="button" onClick={openCreateOrder}
              className="flex items-center gap-2 px-3 sm:px-4 py-2.5 bg-[#0f5931] text-white rounded-xl text-sm font-semibold hover:bg-[#12693a] transition-colors whitespace-nowrap ml-auto">
              ...
              <div className="px-4 pt-4 pb-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    {!o.courier_sent && (
                      <input type="checkbox" checked={selectedOrders.has(o.id)} onChange={() => toggleSelectOrder(o.id)} className="w-4 h-4 accent-[#0f5931] shrink-0 mt-0.5" />
                    )}
                  ...
                  <p className="text-lg font-bold text-[#0f5931] shrink-0">৳{toBn(o.total)}</p>
                </div>
              </div>
              {/* Badges row */}
                  ...
                  <tr>
                    <th className="px-2 py-3 w-8">
                      <input type="checkbox" checked={selectedOrders.size > 0 && selectedOrders.size >= filtered.filter(o => !o.courier_sent).length}
                        onChange={toggleSelectAll} className="w-4 h-4 accent-[#0f5931]" />
                    </th>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 whitespace-nowrap">
                      <span className="inline-flex items-center gap-1.5">
                        Courier
                        <button type="button" onClick={() => setBulkRefreshOpen(true)} className="p-0.5 text-gray-400 hover:text-[#0f5931] hover:bg-[#0f5931]/10 rounded transition-colors" title="Bulk refresh courier status">
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 whitespace-nowrap">
                      <span className="inline-flex items-center gap-1.5">
                        Score
                        <button type="button" onClick={() => setBulkScoreOpen(true)} className="p-0.5 text-gray-400 hover:text-[#0f5931] hover:bg-[#0f5931]/10 rounded transition-colors" title="Bulk check scores">
                          <FiRefreshCw className="w-3 h-3" />
                        </button>
                      </span>
                    </th>
                      ...
                      <Fragment key={o.id}>
                        <tr className="hover:bg-gray-50 transition-colors">
                          <td className="px-2 py-3">
                            {!o.courier_sent && (
                              <input type="checkbox" checked={selectedOrders.has(o.id)} onChange={() => toggleSelectOrder(o.id)} className="w-4 h-4 accent-[#0f5931]" />
                            )}
                          </td>
                                ...
                                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                                  <div className="bg-gray-50/80 border-t border-gray-100 px-6 py-4">
                                          ...
                                          <div key={item.id} className="flex items-center gap-3 bg-white rounded-xl px-3 py-2.5 border border-gray-100 shadow-sm">
                                            <div className="w-14 h-14 rounded-xl overflow-hidden bg-gray-100 shrink-0 relative cursor-pointer"
                                              ...
                                              {item.product?.image ? (
                                                ...
                                                <div className="w-full h-full flex items-center justify-center bg-[#0f5931]/10 text-[#0f5931]">
                                            ...
                                            <p className="text-sm font-bold text-[#0f5931] shrink-0">৳{toBn(item.price * item.quantity)}</p>
                                          </div>
                                        ))}
                                      </div>
                                    )}
                                    <div className="flex items-center justify-end gap-4 mt-3 pt-2 border-t border-gray-200/60 text-xs text-gray-400">
                                      ...
                                      <span className="text-sm font-bold text-[#0f5931]">৳{toBn(o.total)}</span>
                                    </div>
                                  </div>
                                </motion.div>
                              </td>
                            ...
                            <div className="flex items-center gap-2 text-sm">
                              ...
                              {editingCN?.orderId === o.id ? (
                                <input value={editingCN.value} onChange={(e) => setEditingCN({ ...editingCN, value: e.target.value })}
                                  className="font-mono text-sm px-2 py-1 border border-gray-300 rounded-lg focus:border-[#0f5931] focus:outline-none w-36" autoFocus />
                              ) : (
      ...
      <Modal open={!!editOrder} onClose={() => setEditOrder(null)} title={editOrder ? `${t("modal.editOrder")} #${toBn(editOrder.id)}` : ""} size="lg">
        {editOrder && (
              <form onSubmit={handleEditSubmit} className="p-6 space-y-4">
                ...
                <div className="space-y-3">
                  <p className="text-xs font-bold text-[#0f5931] uppercase tracking-wide">{t("misc.customerInfo")}</p>
                ...
                <div className="space-y-3">
                  <p className="text-xs font-bold text-[#0f5931] uppercase tracking-wide">{t("misc.productItems")}</p>
                  <div className="relative">
                    <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                    <input type="text" value={editProductSearch} onChange={(e) => setEditProductSearch(e.target.value)}
                      placeholder={t("search.products")} className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:border-[#0f5931] focus:outline-none" />
                        ...
                        {allProducts
                          ...
                          .map((p) => (
                            <button type="button" key={p.id} onClick={() => {
                              ...
                              <span className="text-[#0f5931] font-semibold shrink-0">৳{p.price}</span>
                            </button>
                          ))}
                ...
                {(() => {
                  ...
                  return (
                    <div className="bg-gray-50 rounded-xl p-3 space-y-1 text-sm">
                      ...
                      <div className="flex justify-between font-bold text-base border-t border-gray-200 pt-1 mt-1"><span>{t("th.total")}</span><span className="text-[#0f5931]">৳{Math.max(0, sub + ship - disc)}</span></div>
                    </div>
                  );
                })()}
        ...
        <div className="p-6">
          {bulkRefreshing ? (
            <div className="space-y-4">
              <div className="flex justify-center">
                <div className="w-16 h-16 rounded-full bg-[#0f5931]/10 flex items-center justify-center">
                  <FiRefreshCw className="w-7 h-7 text-[#0f5931] animate-spin" />
                </div>
              </div>
              <div className="text-center">
                ...
                <div className="mt-3 w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                  <div className="bg-[#0f5931] h-full rounded-full transition-all duration-300" style={{ width: `${bulkRefreshProgress.total > 0 ? (bulkRefreshProgress.done / bulkRefreshProgress.total) * 100 : 0}%` }} />
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2.5">
                <button type="button" onClick={() => handleBulkCourierRefresh("page")}
                  className="w-full flex items-center gap-3 p-4 rounded-xl border-2 border-gray-100 hover:border-[#0f5931] hover:bg-[#0f5931]/5 transition-all group">
                  <div className="w-10 h-10 rounded-lg bg-blue-100 flex items-center justify-center group-hover:bg-[#0f5931]/10 transition-colors">
                    <FiRefreshCw className="w-5 h-5 text-blue-600 group-hover:text-[#0f5931] transition-colors" />
                  </div>
                ...
                <button type="button" onClick={() => handleBulkCourierRefresh("pending")}
                  className="w-full flex items-center gap-3 p-4 rounded-xl border-2 border-gray-100 hover:border-[#0f5931] hover:bg-[#0f5931]/5 transition-all group">
                  <div className="w-10 h-10 rounded-lg bg-orange-100 flex items-center justify-center group-hover:bg-[#0f5931]/10 transition-colors">
                    <FiTruck className="w-5 h-5 text-orange-600 group-hover:text-[#0f5931] transition-colors" />
                  </div>
                  <div className="text-left">
          ...
          {bulkScoreChecking ? (
            <div className="space-y-4">
              ...
              <div className="text-center">
                ...
                <div className="mt-3 w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                  <div className="bg-[#0f5931] h-full rounded-full transition-all duration-300" style={{ width: `${bulkScoreProgress.total > 0 ? (bulkScoreProgress.done / bulkScoreProgress.total) * 100 : 0}%` }} />
                </div>
              </div>
            </div>
          ) : (
            <div className="space-y-4">
              ...
              <div className="space-y-2.5">
                ...
                <button type="button" onClick={() => handleBulkScoreCheck("all")}
                  className="w-full flex items-center gap-3 p-4 rounded-xl border-2 border-gray-100 hover:border-[#0f5931] hover:bg-[#0f5931]/5 transition-all group">
                  <div className="w-10 h-10 rounded-lg bg-orange-100 flex items-center justify-center group-hover:bg-[#0f5931]/10 transition-colors">
                    <FiCheckCircle className="w-5 h-5 text-orange-600 group-hover:text-[#0f5931] transition-colors" />
                  </div>
                  <div className="text-left">
