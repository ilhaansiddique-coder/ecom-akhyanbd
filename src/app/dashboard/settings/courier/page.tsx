            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-4">
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="checkbox" checked={form.steadfast_enabled === "true"} onChange={(e) => setForm(p => ({ ...p, steadfast_enabled: e.target.checked ? "true" : "false" }))} className="w-4 h-4 accent-[#0f5931]" />
                  {lang === "en" ? "Courier Active" : "কুরিয়ার সক্রিয়"}
                </label>
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="checkbox" checked={form.steadfast_auto_send === "true"} onChange={(e) => setForm(p => ({ ...p, steadfast_auto_send: e.target.checked ? "true" : "false" }))} className="w-4 h-4 accent-[#0f5931]" />
                  {lang === "en" ? "Auto-send Orders" : "অর্ডার অটো-সেন্ড"}
                </label>
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="checkbox" checked={form.steadfast_include_notes === "true"} onChange={(e) => setForm(p => ({ ...p, steadfast_include_notes: e.target.checked ? "true" : "false" }))} className="w-4 h-4 accent-[#0f5931]" />
              ...
              <p className="text-xs text-gray-400">
                {lang === "en" ? "Get API keys from" : "API কী পেতে"} <a href="https://portal.packzy.com" target="_blank" rel="noopener noreferrer" className="text-[#0f5931] underline">Steadfast Portal</a> {lang === "en" ? "" : "এ যান।"}
              </p>
            </div>
          </div>
