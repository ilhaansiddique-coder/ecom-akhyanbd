...
export default function LanguageClient({ initialData }: { initialData?: Record<string, string> }) {
  const { lang } = useLang();
  const [frontendLang, setFrontendLang] = useState(initialData?.site_language ?? "bn");
  const [dashboardLang, setDashboardLang] = useState(initialData?.dashboard_language ?? "en");
  ...
  const LangOption = ({ value, current, onChange, flag, label, desc }: {
    ...
    <label className={`flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all ${current === value ? "border-[var(--primary)] bg-[var(--primary)]/5" : "border-gray-100 hover:border-gray-200"}`}>
      <input type="radio" checked={current === value} onChange={() => onChange(value)} className="accent-[var(--primary)] w-4 h-4" />
      <div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 max-w-xl">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <div className="flex items-center gap-2 mb-4">
              <FiGlobe className="w-5 h-5 text-[var(--primary)]" />
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <div className="flex items-center gap-2 mb-4">
              <FiLayout className="w-5 h-5 text-[var(--primary)]" />
