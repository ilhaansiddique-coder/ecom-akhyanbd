...
interface InitialData { items: FlashSale[] }

export default function FlashSalesClient({ initialData }: { initialData?: InitialData }) {
  const { t } = useLang();
  const [items, setItems] = useState<FlashSale[]>(initialData?.items ?? []);
                  ...
                  {filtered.length === 0 ? (
                    ...
                    filtered.map((item) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-gray-500 whitespace-nowrap">{formatDateTime(item.starts_at)}</td>
                        <td className="px-4 py-3 text-gray-500 whitespace-nowrap">{formatDateTime(item.ends_at)}</td>
