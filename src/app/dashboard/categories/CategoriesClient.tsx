...
interface InitialData { items: Category[] }

export default function CategoriesClient({ initialData }: { initialData?: InitialData }) {
  const { t } = useLang();
  const [items, setItems] = useState<Category[]>(initialData?.items ?? []);
