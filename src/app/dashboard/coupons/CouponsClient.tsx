...
interface InitialData { items: Coupon[] }

export default function CouponsClient({ initialData }: { initialData?: InitialData }) {
  const { t, lang } = useLang();
  const [items, setItems] = useState<Coupon[]>(initialData?.items ?? []);
                  ...
                  {filtered.length === 0 ? (
                    ...
                    filtered.map((item) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 font-semibold text-[var(--primary)] whitespace-nowrap">
