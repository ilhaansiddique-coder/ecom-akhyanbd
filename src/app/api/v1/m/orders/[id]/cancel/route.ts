...
// Cancel an order. Reason is appended to notes (preserving any prior history)
// rather than overwriting the field.
export const POST = withStaff<{ params: Promise<{ id: string }> }>(async (request, { params }) => {
  ...
  try {
    ...
    const updateData: Prisma.OrderUncheckedUpdateInput = { status: "cancelled" };
    if (reason) {
      const line = `Cancelled: ${reason}`;
    ...
    console.error("Mobile order cancel error:", error);
